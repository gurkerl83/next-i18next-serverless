...
export const appWithTranslation = function(WrappedComponent) {
  ...
  class AppWithTranslation extends React.Component<Props> {
      ...
      if (!isServer()) {
        const changeLanguageCallback = (prevLng: string, newLng: string) => {
          ...
          if (
            ...
            const { as, href } = lngPathCorrector(
              config,
              { as: asPath, href: routeInfo },
              newLng
            );
        ...
        i18n.changeLanguage = async (newLng: string, callback = () => null) => {
          ...
          if (
            ...
            const usedNamespaces = Object.entries(
              i18n.reportNamespaces.usedNamespaces
            )
              .filter(x => x[1] === true)
              .map(x => x[0]);
    ...
    static async getInitialProps(ctx) {
      ...
      */
      if (
        typeof config.defaultNS === 'string' &&
        !namespacesRequired.includes(config.defaultNS)
      ) {
      ...
      */
      if (req && req.i18n) {
        ...
        */
        ...
        const languagesToLoad = lngsToLoad(
          initialLanguage,
          fallbackLng,
