...
describe('create configuration in non-production environment', () => {
  ...
  it('throws if userConfig.localeSubpaths is a boolean', () => {
    expect(() => createConfig(null, { localeSubpaths: 'string' })).toThrow(
  ...
  it('throws if defaultNS does not exist', () => {
    ...
    evalFunc.mockImplementation(() => ({
      readdirSync: jest
        .fn()
        .mockImplementation(() => ['universal', 'file1', 'file2']),
      existsSync: jest.fn().mockImplementation(() => false)
    }));

    expect(() => createConfig(null, {})).toThrow(
  ...
  describe('server-side', () => {
    ...
    it('creates default non-production configuration', () => {
      ...
      expect(config.interpolation.format('format me', 'uppercase')).toEqual(
        'FORMAT ME'
      );
      ...
      expect(config.detection.order).toEqual([
        'cookie',
        'header',
        'querystring'
      ]);
      ...
      expect(config.backend.loadPath).toEqual(
        '/home/user/static/locales/{{lng}}/{{ns}}.json'
      );
      expect(config.backend.addPath).toEqual(
        '/home/user/static/locales/{{lng}}/{{ns}}.missing.json'
      );
    });
    ...
    it('creates custom non-production configuration', () => {
      ...
      const config = createConfig(null, userConfigServerSide);
      ...
      expect(config.backend.loadPath).toEqual(
        '/home/user/static/translations/{{ns}}/{{lng}}.json'
      );
      expect(config.backend.addPath).toEqual(
        '/home/user/static/translations/{{ns}}/{{lng}}.missing.json'
      );
    });
    ...
    it('preserves config.ns, if provided in user configuration', () => {
      ...
      const config = createConfig(null, { ns: ['common', 'ns1', 'ns2'] });
    ...
    describe('localeExtension config option', () => {
      it('is set to JSON by default', () => {
        const config = createConfig(null, userConfig);
        expect(config.backend.loadPath).toEqual(
          '/home/user/static/translations/{{ns}}/{{lng}}.json'
        );
        expect(config.backend.addPath).toEqual(
      ...
      it('accepts any string and modifies backend paths', () => {
        ...
        expect(config.backend.loadPath).toEqual(
          '/home/user/static/translations/{{ns}}/{{lng}}.test-extension'
        );
        expect(config.backend.addPath).toEqual(
          '/home/user/static/translations/{{ns}}/{{lng}}.missing.test-extension'
        );
      });
    });
  });
  ...
  const runClientSideTests = () => {
    ...
    it('creates custom client-side non-production configuration', () => {
      const config = createConfig(null, userConfigClientSide);
      ...
      expect(config.backend.loadPath).toEqual(
        '/static/translations/{{ns}}/{{lng}}.json'
      );
      expect(config.backend.addPath).toEqual(
        '/static/translations/{{ns}}/{{lng}}.missing.json'
      );
    });
    ...
    describe('localeExtension config option', () => {
      ...
      it('accepts any string and modifies backend paths', () => {
        ...
        expect(config.backend.loadPath).toEqual(
          '/static/translations/{{ns}}/{{lng}}.test-extension'
        );
        expect(config.backend.addPath).toEqual(
          '/static/translations/{{ns}}/{{lng}}.missing.test-extension'
        );
      });
    });
  };
  ...
  describe('https://github.com/isaachinman/next-i18next/issues/134', () => {
    describe('if user specifies a default language and not a fallbackLng', () => {
      ...
      it('fallbackLng === false in development', () => {
        ...
        const config = createConfig(null, userConfigDeNoFallback);
